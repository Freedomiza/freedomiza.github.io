    ...
    return e = o, n = [{
      ...
      value: function () {
        var e,
            ...
            a,
            r = this;
        r.paused || (r.keep || r.active || (r.mouseX = Math.abs(r.mouseX - r.mouseX0) < 1 ? r.mouseX0 : (r.mouseX + r.mouseX0) / 2, r.mouseY = Math.abs(r.mouseY - r.mouseY0) < 1 ? r.mouseY0 : (r.mouseY + r.mouseY0) / 2), e = -(Math.min(Math.max(-r.mouseY, -r.size), r.size) / r.radius) * r.maxSpeed, t = Math.min(Math.max(-r.mouseX, -r.size), r.size) / r.radius * r.maxSpeed, Math.abs(e) <= 0.01 && Math.abs(t) <= 0.01 || (n = Math.PI / 180, a = [Math.sin(e * n), Math.cos(e * n), Math.sin(t * n), Math.cos(t * n)], r.items.forEach(function (e) {
          var t = e.x,
              n = e.y * a[1] + e.z * -a[0],
              i = e.y * a[0] + e.z * a[1],
              o = t * a[3] + i * a[2],
          ...
          o = (1 < (o = t * t - 0.25) ? 1 : o).toFixed(3), n = e.el, i = (e.x - n.offsetWidth / 2).toFixed(2), t = (e.y - n.offsetHeight / 2).toFixed(2), e = "translate3d(".concat(i, "px, ").concat(t, "px, 0) scale(").concat(e.scale, ")");
