...
const updateItemPosition = (item: any, sc: number[], depth: number) => {
  ...
  // @ts-ignore
  const left = (newItem.x - (itemEl?.offsetWidth ?? 0) / 2).toFixed(2);
  // @ts-ignore
  const top = (newItem.y - (itemEl?.offsetHeight ?? 0) / 2).toFixed(2);
...
const defaultState: tagSphereProps = {
  texts: [
    ...
    <img
      key="586dbbf2-0930-4443-b5ca-26cbd11a8ce0"
      width={50}
...
export default function TagSphere(props: any) {
  ...
  const init = useCallback(() => {
    ...
    next();
  }, [initialDirection, initialSpeed, next]);
